		...
		if (
			...
			try {
				...
				while (i < actual.length) {
					snt[i].DateTimeUTC = actual[i].DateTime;
					snt[i].ActualTotalLoadValue = actual[i].TotalLoadValue;
					snt[i].UpdateTimeUTC = actual[i].UpdateTime;
					i = i + 1;
				}

				try {
					const resolutions = await Resolution.find({
						Id: actual[0].ResolutionCodeId
					}).select({ ResolutionCodeText: 1, _id: 0 });

					const maps = await Map.find({ Id: actual[0].MapCodeId }).select({
						MapCodeText: 1,
						_id: 0
					});

					const areas = await Area.find({
						Id: actual[0].AreaTypeCodeId
...
router.get(
	...
	async (req, res) => {
		if (
			...
			try {
				const actual = await Actual.aggregate([
					{
						$match: {
							AreaName: paramData.areaName,
							Year: paramData.year,
				...
				var i = 0;
				while (i < actual.length) {
					snt[i].Day = actual[i]._id.Day;
					snt[i].ActualTotalLoadByDayValue = actual[i].count;
					i = i + 1;
				}

				try {
					const resolutions = await Resolution.find({
						Id: actual[0]._id.ResolutionCodeId
					}).select({ ResolutionCodeText: 1, _id: 0 });

					const maps = await Map.find({
						Id: actual[0]._id.MapCodeId
					}).select({ MapCodeText: 1, _id: 0 });

					const areas = await Area.find({
						Id: actual[0]._id.AreaTypeCodeId
					}).select({ AreaTypeCodeText: 1, _id: 0 });
	...
	async (req, res) => {
		if (
			...
			try {
				...
				var i = 0;
				while (i < actual.length) {
					snt[i].Month = actual[i]._id.Month;
					snt[i].ActualTotalLoadByMonthValue = actual[i].count;
					i = i + 1;
				}

...
export default class DayAheadTotalLoadForecast extends Component {
	constructor(props) {
		super(props);
		this.state = {
			areaname: '',
	...
	handleSubmit(event) {
		...
		axios
			.get(
				`http://localhost:8765/energy/api/DayAheadTotalLoadForecast/${
					this.state.areaname
			...
			.then((res) => {
				const realDatas = res.data.map((realData) => {
					console.log(realData.ActualTotalLoadValue);
					if (this.state.datetype == 'date') {
						return realData.DayAheadTotalLoadForecastValue;
					}
					if (this.state.datetype == 'month') {
						return realData.DayAheadTotalLoadForecastByDayValue;
					}
				});
				this.setState({
					response: res.data,
	...
	render() {
		return (
			<div>
				<Container>
					<br />
					<h3>Day Ahead Total Load Forecast Value</h3>
					<br />

...
export default class ActualTotalLoad extends Component {
	constructor(props) {
		super(props);
		this.state = {
			areaname: '',
	...
	handleSubmit(event) {
		...
		axios
			.get(
				`http://localhost:8765/energy/api/ActualTotalLoad/${
					this.state.areaname
			...
			.then((res) => {
				const realDatas = res.data.map((realData) => {
					...
					if (this.state.datetype == 'date') {
						return realData.ActualTotalLoadValue;
					}
					if (this.state.datetype == 'month') {
						return realData.ActualTotalLoadByDayValue;
					}
				});
				this.setState({
					response: res.data,
